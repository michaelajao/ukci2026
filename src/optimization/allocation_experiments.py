...
# Policies shown in the paper's two-panel Table 2, in row order
# (status quo is kept in the CSVs but dropped from the manuscript table).
_TABLE2_PANEL_ORDER = (
    "Population-proportional",
    "Demand-proportional",
    "Greedy shortage-first",
...
def build_two_panel_allocation_table() -> str:
    """Emit the manuscript two-panel Table 2 (LaTeX booktabs tabular):
    a peak-origin stress panel (first Omicron origin) above an all-origin mean
    panel (32 rolling origins). Source-of-truth for the hand-set table in the
    paper; the inline numbers there must match this file.
    ...
    for pol in _TABLE2_PANEL_ORDER:
        r = allo.loc[pol]
        lines.append(_row(pol, r["total_surge_mean"], r["expected_unmet_mean"],
                          r["worst_case_unmet_mean"], r["transfer_mean"]))
    lines += ["\\bottomrule", "\\end{tabular}", ""]

    text = "\n".join(lines)
    (OUT_DIR / "table2_two_panel.tex").write_text(text, encoding="utf-8")
    return text
...
def run_allocation_revision_main() -> int:
    ...
    if (OUT_DIR / "table2_allocation.csv").exists():
        build_two_panel_allocation_table()
        print(f"Wrote {OUT_DIR / 'table2_two_panel.tex'}")
    else:
        print("Skipped two-panel Table 2: run ukci-run-allocation-e2 first "
              "to produce table2_allocation.csv (peak-origin panel).")
