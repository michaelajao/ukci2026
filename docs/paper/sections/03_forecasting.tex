...
and $a^{h,q},g^{h,q},\psi^q\in(0,1)$ are learnable scalars (distinct from
the SEIRD constants $\alpha,\gamma_{c,r}$ of \Cref{eq:seird}).
...
leakage-free steps. \emph{(i)~Bias recalibration:} at each rolling origin
$t$ and horizon $h$, subtract the mean of the model's last $K{=}3$
$h$-step errors whose target date is already observed ($<t$)---the
adaptation a per-origin refit gives for free. \emph{(ii)~Persistence
blend:} blend the recalibrated point forecast with seasonal-naive($7$)
using a per-horizon weight chosen by an expanding window over realised
past origins, so long horizons lean on persistence. Both use only
information available at $t$; the $80\%$ interval is left as produced by
the quantile heads, so the downstream allocation is unchanged.
