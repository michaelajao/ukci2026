...
constants (values from~\cite{ajao2025hybrid}, Table~1) and
$\beta_r,\gamma_{c,r},\delta_{c,r},\eta_r$ are time-varying
region-specific parameters (transmission, critical-care
recovery and death, and immunity waning) learned from data.
...
and $C$ (mechanical-ventilation, i.e.\ critical-care, bed occupancy),
...
With only $H$ and $C$ observed regionally, the four learnable parameters
$(\beta_r, \gamma_{c,r}, \delta_{c,r}, \eta_r)$ are not jointly
identifiable ($\gamma_{c,r}$ and $\delta_{c,r}$ appear additively in
$\dot C_r$), so we make no interpretability claim for individual
trajectories: the four-channel output is consumed as a learned feature
vector by the GRU, whose signal the ``w/o PINN params'' ablation
confirms (\Cref{ssec:e1}; $h{=}14$ RMSE $14.46\!\to\!27.25$).
Structural identifiability under $\{H,C\}$ observation is future work.
...
latent $S,E,I_a,I_s,R,D$ that are never directly observed), so the
head receives the full physics-inferred state rather than the two
fitted channels alone, and $\tilde y_{r,i}$ is the observed target
z-scored with training-period statistics only (no test-set leakage).
A two-layer
...
leakage-free steps. The first recalibrates the bias, subtracting at
...
with persistence, mixing the recalibrated point forecast with
seasonal-naive($7$) under a per-horizon weight
$\alpha\in\{0,0.1,\dots,1\}$ chosen on an expanding window of realised
past origins (fitted once at least seven origin--region pairs are
realised; until then $\alpha{=}1$, pure model), so long horizons lean on
persistence. Both use only information available at $t$, and the
$80\%$ interval is left as produced by the quantile heads, so the
downstream allocation is unchanged.
