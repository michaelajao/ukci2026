...
epidemic with an eight-compartment $SEI_aI_sHCRD$ system. Let
$U^r(t)=(S_r,E_r,I_{a,r},I_{s,r},H_r,C_r,R_r,D_r)$ denote susceptible,
exposed, asymptomatic infectious, symptomatic infectious, hospitalised,
critical-care, recovered, and deceased compartments. \Cref{fig:seird}
...
network $U^r_{NN}\!:t\mapsto U^r(t)$ (five hidden layers of $20$ units)
and a parameter network
$X^r_{NN}\!:t\mapsto(\beta_r,\gamma_{c,r},\delta_{c,r},\eta_r)$ (three
hidden layers of $20$ units), with tanh hidden activations and sigmoid
outputs. The loss combines a data fit on observable compartments
...
\subsection{Pinball Loss and Decision-Aware Asymmetry}\label{ssec:loss}
...
under-prediction nine-to-one. We call the $q^{(0.9)}$ pinball
\emph{decision-aware} because its asymmetry matches the downstream cost
asymmetry (an under-bed risks a stranded patient; an over-bed only
wastes pre-positioned capacity)~\cite{elmachtoub2022smart}, not because
the forecaster is coupled to the allocation solver. We report $\hat y^{0.5}$
