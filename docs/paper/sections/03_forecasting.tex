...
For each region $r \in \R$ with a population $N_r$, we model the local
...
The fixed clinical constants $(\alpha,\rho,d_s,d_a,d_H)$ take the values
in \cite[Table~1]{ajao2025hybrid}, whereas $\omega$, $\mu$ and $\phi$ are
calibrated here; rates have units of
$\text{day}^{-1}$ and probabilities lie in $[0,1]$. Data determine the
time-varying, region-specific transmission, critical-care recovery,
death, and immunity-waning rates
$(\beta_r,\gamma_{c,r},\delta_{c,r},\eta_r)$.
...
\draw[edge] (E) --
  node[elabel, above, pos=0.48, yshift=2pt] {$\alpha(1-\rho)$} (Ia);
...
\draw[edge] (H.south east) --
  node[elabel, below, pos=0.40] {$(1-\phi)d_H$} (R.north west);
...
population-normalised. We pre-train each region's PINN for $1{,}500$ Adam steps with
...
the ``w/o PINN params'' ablation confirms it carries signal (\Cref{ssec:e1};
...
Once pre-trained, the PINN is frozen and evaluated, never to be refit, at every
origin. The downstream GRU and anchor absorb extrapolation beyond its
Alpha/Delta fit. For each lookback window of length
...
training and validation statistics only.
A two-layer
...
\end{equation}
where $\tilde y_{r,t}$ is the latest z-scored observation,
$v_{r,t}^{(7)}=\tilde y_{r,t}-\tilde y_{r,t-7}$ is its weekly slope,
and $a^{h,q},g^{h,q},\psi^q\in(0,1)$ are learned. The damping exponent
$\psi^q$ limits long-horizon slope overshoot (initialised at $0.88$ for
$q{=}0.5$ and $0.5$ otherwise). We then invert standardisation.

\subsection{Multi-Quantile Pinball Loss}\label{ssec:loss}
...
collapses to L1 (point accuracy). Splitting the kernel into its two arms,
$\max(q e,(q{-}1)e)=q\max(e,0)+(1{-}q)\max(-e,0)$, exposes it as a
convex newsvendor loss. The $q{=}0.9$ branch penalises
under-prediction nine-to-one and is therefore cost-relevant to shortage;
the $q{=}0.1$ branch has the equal and opposite asymmetry. Because all
three branches receive equal weight, their aggregate training objective
is not directionally asymmetric. We therefore claim a cost-relevant
upper quantile, not an overall cost-asymmetric loss. Training is not
through the allocation solver~\cite{elmachtoub2022smart};
$\hat q^{(0.5)}$ is the raw point forecast and
$[\hat q^{(0.1)},\hat q^{(0.9)}]$ the raw $80\%$ interval.
...
over-prediction. At each origin $t$ and horizon $h$, we subtract the mean
of the last $K{=}3$ realised $h$-step errors, then blend with
...
realised). Both steps use only information available at $t$.

Let $\hat{\mathbf q}_{r,t}^{h}=(\hat q^{(0.1)},\hat q^{(0.5)},
\hat q^{(0.9)})$ denote the raw heads and
$\tilde y_{r,t}^{h,\mathrm{pt}}$ the recalibrated point forecast.
\Cref{tab:forecast_accuracy} evaluates both point estimates; the
calibration table and every LP instead use $\hat{\mathbf q}$ unchanged
(apart from clipping negative demand to zero). Thus, recalibrated RMSE
gains do not enter the allocation results.
