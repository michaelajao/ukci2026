...
(LSTM) network with lagged mobility features and Monte-Carlo-dropout
...
bilinear mixed-integer program (MIP) for vaccination-site location,
while Luo and Stellato~\cite{luo2024equitable} embed neural-ODE
compartmental dynamics inside a McCormick-relaxed facility-location
model. Our setting differs from
...
a level-and-trend anchor, and a per-origin recalibration
reduce $h{=}14$ RMSE by $25\%$ relative to a
refitted ARIMA model and $42\%$
