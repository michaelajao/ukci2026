...
is uniform per bed; the model decides only bed counts and routing. Indices: regions $r,r'\!\in\!\R$; horizons
...
(here-and-now) capacity decision, the planner's only true control, is
the surge allocation $b_{r}\!\in\![0,K_r]$, shared across scenarios and
horizons (beds stood up once serve every checkpoint). The
...
form generalises unchanged to multiple tail scenarios. Because the high
scenario imposes $q^{(0.9)}$ jointly across regions and horizons, the
construction is co-monotone and its tail over-conservative relative to
the true joint distribution; we treat this as an explicit stress-test
choice and examine its efficiency cost in \Cref{ssec:e5_e6}.
